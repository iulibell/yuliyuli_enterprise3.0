...
<script setup lang="ts">
  ...
  const handlePrivateMessage = () => {
    ...
    if (!userId.value) {
      ElMessage.warning('作者信息异常，请刷新后重试');
      return;
    }
    router.push({
      path: '/messages',
  ...
  const fetchAuthorData = async () => {
    ...
    let effectiveUserId = (userId.value || '').trim();
    if (!effectiveUserId || effectiveUserId === '0' || effectiveUserId === 'undefined') {
      effectiveUserId = '';
    }

    // 路由里没带有效 userId 时，用作者昵称反查（列表里可能未带 userId）
    if (!effectiveUserId && authorName.value) {
      try {
        const nameRes = await axios.get(
          `/api/info/userInfoByName/${encodeURIComponent(authorName.value)}`
        );
        if (nameRes.data?.code === 200 && nameRes.data.data?.userId != null) {
          effectiveUserId = String(nameRes.data.data.userId);
          userId.value = effectiveUserId;
        }
      } catch (e) {
        console.warn('按昵称解析作者ID失败:', e);
      }
    }

    // 用户信息：单独失败不影响拉取投稿列表（避免未登录时 userInfo 401 导致整段 catch 清空视频列表）
    if (effectiveUserId) {
      try {
        const userInfoResponse = await axios.get(`/api/info/userInfo/${effectiveUserId}`);
        ...
        console.warn('获取用户信息失败（仍尝试加载视频）:', e);
        authorInfo.value.username = authorName.value;
        if (avatarQuery.value) {
          try {
            authorInfo.value.avatar = decodeURIComponent(avatarQuery.value);
          } catch {
            authorInfo.value.avatar = avatarQuery.value;
          }
        }
      }

      try {
        const videoResponse = await axios.get(`/api/info/authorPage/${effectiveUserId}`);
