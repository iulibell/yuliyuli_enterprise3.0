      ...
      <div class="video-container">
        ...
        <div class="video-player">
          ...
          <div class="video-info">
            ...
            <div class="video-stats">
              <div class="author-info">
                <el-avatar :src="video.authorAvatar" alt="作者" @click="handleAuthorClick(video.authorName)" style="cursor: pointer;" />
                <div class="author-details">
                  <span class="author-name" @click="handleAuthorClick(video.authorName)" style="cursor: pointer;">{{ video.authorName }}</span>
                ...
                <el-button 
                  v-else
                  type="primary" 
                  style="background-color: deeppink; color: white; border-radius: 10px; border: none;"
                  @click="handleFollow"
                  :loading="isFollowing"
                >
                  关注
          ...
          <div class="comment-section">
            ...
            <div class="comment-input">
              ...
              <input type="text" v-model="commentContent" placeholder="添加评论..." v-if="isLoggedIn" />
              <div v-else class="login-prompt">
                登录后才能发表评论
                <el-button type="primary" size="small" @click="handleLogin">去登录</el-button>
              </div>
              <button class="comment-submit" v-if="isLoggedIn" @click="handleSubmitComment">发送</button>
            </div>
        ...
        <div class="recommend-videos">
          ...
          <div class="recommend-list">
            <div v-for="item in hotVideoVOList" :key="item.id" class="recommend-item" @click="handleRecommendClick(item)">
              ...
              <div class="recommend-info">
                ...
                <div class="recommend-stats">{{ formatPlayCount(item.playCount) }} 播放 · {{ item.commentCount }} 评论</div>
              </div>
            </div>
          </div>
        </div>
  ...
  router.push({
    ...
    query: {
      video: JSON.stringify({
        ...
        createTime: typeof item.createTime === 'string' ? item.createTime : (item.createTime?.toISOString() || new Date().toISOString()),
...
const handleCollect = async () => {
  ...
  try {
    ...
    const response = await axios.post('/api/video/collect', {
      videoId: videoId,
      userId: parseInt(userId)
    }, {
      headers: {
  ...
  try {
    ...
    if (response.data) {
      ...
      if (response.data.code === 200) {
        ...
        // 获取热门推荐视频
        hotVideoVOList.value = response.data.data.hotVideoVOList || response.data.data.videoVOList || [];
