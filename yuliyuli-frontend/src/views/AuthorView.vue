    ...
    <main class="author-content">
        ...
        <div class="author-info">
          ...
          <div class="author-buttons">
            <el-button 
              v-if="!isCurrentUser"
              type="primary" 
              :style="isFollowed ? 'background-color: #999; color: white; border-radius: 10px; border: none; margin-right: 10px;' : 'background-color: deeppink; color: white; border-radius: 10px; border: none; margin-right: 10px;'"
              @click="handleFollow"
              :loading="isFollowing"
            >
              {{ isFollowed ? '取消关注' : '关注' }}
            </el-button>
            <el-button v-if="!isCurrentUser" type="default" style="background-color: #f0f0f0; color: #333; border-radius: 10px; border: none; margin-right: 10px;" @click="handlePrivateMessage">
              私信
            </el-button>
            <el-button v-if="isCurrentUser" type="default" style="background-color: #f0f0f0; color: #333; border-radius: 10px; border: none; margin-right: 10px;" @click="handleEditProfile">
            编辑资料
          </el-button>
          <el-button v-if="isCurrentUser" type="default" style="background-color: #f0f0f0; color: #333; border-radius: 10px; border: none;" @click="router.push(`/video-manager?userId=${userId}`)">
            稿件管理
          </el-button>
          </div>
        </div>
      ...
      <div class="author-nav">
        <div class="nav-content">
          <router-link :to="`/author/${userId}/${authorName}`" class="nav-item" :class="{ active: activeTab === 'home' }">主页</router-link>
          <router-link :to="`/author/${userId}/${authorName}/videos`" class="nav-item" :class="{ active: activeTab === 'videos' }">投稿</router-link>
          <router-link :to="`/author/${userId}/${authorName}/playlists`" class="nav-item" :class="{ active: activeTab === 'playlists' }">合集和系列</router-link>
        </div>
      </div>
      ...
      <div class="video-section">
        ...
        <template v-else>
          ...
          <div v-if="videoList.length > 0" class="video-grid">
            <div v-for="video in videoList" :key="video.id" class="video-card" @click="handleVideoClick(video)">
              ...
              <div class="video-info">
                ...
                <div class="video-stats">{{ formatPlayCount(video.playCount) }} 播放 · {{ formatTime(video.createTime) }}</div>
              </div>
            </div>
          </div>
...
const fetchAuthorData = async () => {
  try {
    ...
    // 获取关注状态
    if (isLoggedIn.value && currentUserId.value && userId.value && userId.value !== currentUserId.value) {
      try {
        // 调用后端接口获取关注状态
