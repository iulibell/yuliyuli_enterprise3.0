  ...
  <div class="search-result">
    ...
    <header class="header">
      <div class="header-content">
        ...
        <div class="user-icon">
          <div v-if="isLoggedIn" @click="handleAvatarClick" style="cursor: pointer; display: inline-block;">
        ...
        <div class="user-actions">
          <button style="background-color: deepskyblue; color: white; 
            border-radius: 10px; width: 80px; margin-left: -130px; border: none; height: 33px;" class="upload-btn" @click="router.push('/upload')">投稿</button>
        </div>
      </div>
    </header>
    ...
    <main class="search-content">
      ...
      <div class="category-tabs" v-if="!loading && searchResults.length > 0">
        <div class="tab-item" :class="{ active: activeTab === '综合' }" @click="handleTabClick('综合')">综合</div>
        <div class="tab-item" :class="{ active: activeTab === '视频' }" @click="handleTabClick('视频')">视频</div>
        <div class="tab-item" :class="{ active: activeTab === '番剧' }" @click="handleTabClick('番剧')">番剧</div>
        <div class="tab-item" :class="{ active: activeTab === '影视' }" @click="handleTabClick('影视')">影视</div>
        <div class="tab-item" :class="{ active: activeTab === '直播' }" @click="handleTabClick('直播')">直播</div>
        <div class="tab-item" :class="{ active: activeTab === '专栏' }" @click="handleTabClick('专栏')">专栏</div>
        <div class="tab-item" :class="{ active: activeTab === '用户' }" @click="handleTabClick('用户')">用户</div>
      </div>

      <!-- 排序选项 -->
      <div class="sort-options" v-if="!loading && searchResults.length > 0 && activeTab === '综合'">
        <div class="sort-item" :class="{ active: activeSort === '综合排序' }" @click="handleSortClick('综合排序')">综合排序</div>
        <div class="sort-item" :class="{ active: activeSort === '最多播放' }" @click="handleSortClick('最多播放')">最多播放</div>
        <div class="sort-item" :class="{ active: activeSort === '最新发布' }" @click="handleSortClick('最新发布')">最新发布</div>
        <div class="sort-item" :class="{ active: activeSort === '最多弹幕' }" @click="handleSortClick('最多弹幕')">最多弹幕</div>
        <div class="sort-item" :class="{ active: activeSort === '最多收藏' }" @click="handleSortClick('最多收藏')">最多收藏</div>
        <div class="sort-more">更多筛选 <i class="el-icon-arrow-down"></i></div>
      </div>

      <!-- 搜索结果网格 -->
      <div class="result-grid" v-if="searchResults.length > 0 && activeTab === '综合'">
        <div v-for="video in searchResults" :key="video.url" class="result-card" @click="handleVideoClick(video)">
          <div class="result-cover">
            <img :src="video.cover" alt="视频封面" />
            <div class="result-stats-overlay">
              <span class="play-count"><i class="el-icon-video-camera"></i> 👁 {{ formatPlayCount(video.playCount) }}</span>
              <span class="danmaku-count"><i class="el-icon-chat-dot-round"></i> 💬 {{ video.commentCount }}</span>
            </div>
          </div>
          <div class="result-info">
            <h3 class="result-title">{{ video.title }}</h3>
            <div class="result-meta">
              <span class="result-author" @click.stop="handleAuthorClick(video)">{{ video.authorName }}</span>
            </div>
          </div>
        </div>
      </div>
